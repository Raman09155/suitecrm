...
{if !isset($fields.full_name.value)}
...
{if !isset($fields.user_name.value)}
...
{counter name="panelFieldCount" print=false}
<span id="UserType" class="sugar_field">{if isset($USER_TYPE_READONLY)}{$USER_TYPE_READONLY}{/if}</span>
{/if}
...
{if !isset($fields.photo.value)}
<img src="" style="max-width: {if !$vardef.width}160{else}200{/if}px;" height="{if !$vardef.height}160{else}50{/if}">
{else}
<img src="index.php?entryPoint=download&id={$fields.id.value|default:''}_{$fields.photo.name}{$fields.width.value|default:10}&type={$module}" style="max-width: {if isset($vardef.width)}160{else}200{/if}px;" height="{if isset($vardef.height)}160{else}50{/if}">
{/if}
</span>
{/if}
...
{if !isset($fields.department.value)}
...
{if !isset($fields.messenger_id.value)}
...
{if !isset($fields.address_street.value)}
...
{if !isset($fields.address_city.value)}
...
{if !isset($fields.address_state.value)}
...
{if !isset($fields.address_postalcode.value)}
...
{if !isset($fields.address_country.value)}
